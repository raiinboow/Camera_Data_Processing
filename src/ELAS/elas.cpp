...
#ifdef PROFILE
  CalcTime timer("time",TIME_MSEC,false); 
...
#ifdef PROFILE
  timer.lap("L/R Consistency Check");
#endif
...
#ifdef PROFILE
  timer.lap("Remove Small Segments");
#endif
  gapInterpolation(D1);
  if (!param.postprocess_only_left)
    gapInterpolation(D2);
...
vector<Elas::triangle> Elas::computeDelaunayTriangulation (vector<support_pt> p_support,int32_t right_image) {
  ...
  // inputs
  in.numberofpoints = (int)p_support.size();
  in.pointlist = (float*)malloc(in.numberofpoints*2*sizeof(float));
  k=0;
  if (!right_image) {
    for (int32_t i=0; i<(int32_t)p_support.size(); i++) {
      in.pointlist[k++] = (float)p_support[i].u;
      in.pointlist[k++] = (float)p_support[i].v;
    }
  } else {
    for (int32_t i=0; i<(int32_t)p_support.size(); i++) {
      in.pointlist[k++] = (float)(p_support[i].u-p_support[i].d);
      in.pointlist[k++] = (float)p_support[i].v;
    }
  }
...
void Elas::createGrid(vector<support_pt> p_support,int32_t* disparity_grid,int32_t* grid_dims,bool right_image) {
  ...
  for (int32_t i=0; i<(int32_t)p_support.size(); i++) {
    ...
    for (int32_t d=d_min; d<=d_max; d++) {
      int32_t x;
      if (!right_image)
        x = (int32_t)floor((float)(x_curr/param.grid_size));
      else
        x = (int32_t)floor((float)(x_curr-d_curr)/(float)param.grid_size);
      int32_t y = (int32_t)floor((float)y_curr/(float)param.grid_size);
...
inline void Elas::findMatch(int32_t &u,int32_t &v,float &plane_a,float &plane_b,float &plane_c,
  ...
  // set disparity value
  if (min_d>=0) *(D+d_addr) = (float)min_d; // MAP value (min neg-Log probability)
...
void Elas::computeDisparity(vector<support_pt> p_support,vector<triangle> tri,int32_t* disparity_grid,int32_t *grid_dims,
  ...
  for (uint32_t i=0; i<tri.size(); i++) {
    ...
    float tri_u[3];
    if (!right_image) {
      tri_u[0] = (float)p_support[c1].u;
      tri_u[1] = (float)p_support[c2].u;
      tri_u[2] = (float)p_support[c3].u;
    } else {
      tri_u[0] = (float)(p_support[c1].u-p_support[c1].d);
      tri_u[1] = (float)(p_support[c2].u-p_support[c2].d);
      tri_u[2] = (float)(p_support[c3].u-p_support[c3].d);
    }
...
void Elas::removeSmallSegments (float* D) {
  ...
  if (param.subsampling) {
    D_width        = width/2;
    D_height       = height/2;
    D_speckle_size = (int32_t)sqrt((float)param.speckle_size)*2;
  }
...
StereoEfficientLargeScale::StereoEfficientLargeScale(int mindis, int dispRange)
{
	minDisparity = mindis;
